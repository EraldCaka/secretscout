...
pub const Scanner = struct {
    ...
    pub fn init(allocator: Allocator, config: types.Config) Scanner {
        return .{
            .allocator = allocator,
            .config = config,
            .findings = std.ArrayList(types.Finding).init(allocator),
        };
    }
    ...
    pub fn scanLineForTest(self: *Scanner, path: []const u8, line_number: usize, line: []const u8) !void {
        try self.scanLine(path, line_number, line);
    }

    fn scanPath(self: *Scanner, path: []const u8) !void {
        if (std.fs.cwd().openDir(path, .{ .iterate = true, .access_sub_paths = true })) |opened_dir| {
            var dir = opened_dir;
            dir.close();
    ...
    fn addFinding(
        ...
        const preview_copy = if (rule == .private_key_marker)
            try self.allocator.dupe(u8, preview_source)
        else
            try preview.redact(self.allocator, preview_source);
